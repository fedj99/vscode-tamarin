...
export function go_to_definition_command(context: vscode.ExtensionContext) {
    const provide_go_toCodeAction = vscode.commands.registerCommand("Search definition", async () => {
        ...
        if (activeFile) {
            ...
            similar_symbols = [];
            for (const symbol of symbolTable.symbolTable.getSymbols()) {
                if (
                    symbol.name === corresponding_symbol?.name &&
                    (corresponding_symbol?.declaration === DeclarationType.ActionF ||
                        corresponding_symbol?.declaration === DeclarationType.LinearF ||
                        corresponding_symbol?.declaration === DeclarationType.PersistentF ||
                        corresponding_symbol?.declaration === DeclarationType.Lemma ||
                        corresponding_symbol?.declaration === DeclarationType.Rule)
                ) {
                    similar_symbols.push(symbol);
                } else if (
                    symbol.name === corresponding_symbol?.name &&
                    corresponding_symbol?.declaration === DeclarationType.NARY
                ) {
                    similar_symbols.push(symbol);
                    break;
                }
            ...
            if (similar_symbols.length > 0) {
                ...
                const navigateToNextSymbolCommand = vscode.commands.registerCommand(
                    "myExtension.navigateToNextSymbol",
                    () => {
