    ...
    const provideRenameCodeAction = vscode.commands.registerCommand("Rename", async () => {
        ...
        if (activeFile) {
            ...
            for (let k = 0; k < symbolTable.symbolTable.getSymbols().length; k++) {
                const current_symbol = symbolTable.symbolTable.getSymbol(k);
                if (
                    ...
                    if (
                        corresponding_symbol?.context === current_symbol.context &&
                        chosenSymbolName === current_symbol.name
                    ) {
                        replace_count++;
                    ...
                    if (
                        current_symbol.associated_qf?.id === corresponding_symbol?.associated_qf?.id &&
                        current_symbol.name === chosenSymbolName
                    ) {
                        replace_count++;
                        replace_symbol(current_symbol, activeEditor, newName, edit);
                    }
                } else if (
                    current_symbol.declaration === DeclarationType.LEquationVariable ||
                    current_symbol.declaration === DeclarationType.REquationVariable
                ) {
                    if (
                        corresponding_symbol?.context.id === current_symbol.context.id &&
                        chosenSymbolName === current_symbol.name
                    ) {
                        replace_count++;
                        replace_symbol(current_symbol, activeEditor, newName, edit);
                    ...
                    current_symbol.declaration === DeclarationType.LMacroVariable ||
                    current_symbol.declaration === DeclarationType.RMacroVariable
                ) {
                    if (
                        corresponding_symbol?.context.id === current_symbol.context.id &&
                        chosenSymbolName === current_symbol.name
                    ...
                    current_symbol.name === chosenSymbolName &&
                    current_symbol.declaration === corresponding_symbol?.declaration
                ) {
                    replace_count++;
                    replace_symbol(current_symbol, activeEditor, newName, edit);
                }
