...
function build_error_display(
    ...
    editeur: vscode.TextEditor,
    diags: vscode.Diagnostic[],
    message: string,
): vscode.Diagnostic {
    const start = node.startIndex;
    const end = node.endIndex;
    const positionStart = editeur.document.positionAt(start);
    const positionEnd = editeur.document.positionAt(end);
...
function build_warning_display(
    node: Parser.SyntaxNode,
    editeur: vscode.TextEditor,
    diags: vscode.Diagnostic[],
    message: string,
...
export function check_reserved_facts(
    node: Parser.SyntaxNode,
            ...
            if (fact_name === ReservedFacts[0]) {
                ...
                if (child.child(2)?.children && get_arity(child.child(2)?.children) !== 1) {
                    build_error_display(
                        ...
                        editor,
                        diags,
                        "Error: incorrect arity for Fr fact, only 1 argument expected",
                    );
                }
                ...
                if (child.child(2)?.children && get_arity(child.child(2)?.children) !== 1) {
                    build_error_display(
                        ...
                        editor,
                        diags,
                        "Error: incorrect arity for In fact, only 1 argument expected",
                    );
                }
                ...
                if (child.child(2)?.children && get_arity(child.child(2)?.children) !== 1) {
                    build_error_display(
                        ...
                        editor,
                        diags,
                        "Error: incorrect arity for Out fact, only 1 argument expected",
                    );
                }
            } else if (
                (fact_name === ReservedFacts[3] || fact_name === ReservedFacts[4] || fact_name === ReservedFacts[5]) &&
                node.parent?.grammarType === "simple_rule"
            ) {
                ...
                if (get_arity(child.child(2)?.children) != 2) {
                    build_error_display(
                        ...
                        editor,
                        diags,
                        "Error : incorrect arity for diff fact, 2 arguments expected",
                    );
                }
            }
            ...
            if (fact_name_node !== null && fact_name_node !== undefined) {
                ...
                if (fact_name === ReservedFacts[6]) {
                    if (node.grammarType === DeclarationType.Equation || node.grammarType === "mset_term") {
                        build_warning_display(
                            child,
                            editor,
                            diags,
                            "Warning  :  diff fact cannot be used in an equation",
                        );
                    }
                }
            }
...
function check_variables_type_is_consistent_inside_a_rule(
    ...
    for (let i = 0; i < symbol_table.getSymbols().length; i++) {
        ...
        if (
            current_symbol.declaration === DeclarationType.PRVariable ||
            current_symbol.declaration === DeclarationType.CCLVariable ||
            current_symbol.declaration === DeclarationType.ActionFVariable
        ) {
            for (let j = 0; j < symbol_table.getSymbols().length; j++) {
                if (
                    ...
                    if (
                        current_symbol.context === symbol_table.getSymbol(j).context &&
                        current_symbol.name === symbol_table.getSymbol(j).name
                    ) {
                        if (current_symbol.type === symbol_table.getSymbol(j).type) {
                            ...
                            build_error_display(
                                ...
                                editor,
                                diags,
                                "Error: inconsistent variables, variables with the same name in the same rule must have same types ",
                            );
                            break;
                        }
                    } else {
        ...
        // Check about the equation variables
            ...
            for (const symbol of symbol_table.getSymbols()) {
                if (
                    symbol.declaration === DeclarationType.LEquationVariable &&
                    symbol.name === current_symbol.name &&
                    symbol.context === current_symbol.context
                ) {
                    isbreak = true;
                    break;
                }
            ...
            if (!isbreak) {
                build_error_display(
                    ...
                    editor,
                    diags,
                    "Error : this variable doesn't exist on the left side of the equation",
                );
            }
        }
...
function check_case_sensitivity(
    ...
    for (let i = 0; i < symbol_table.getSymbols().length; i++) {
        const current_symbol = symbol_table.getSymbol(i);
        if (
            current_symbol.declaration === DeclarationType.LinearF ||
            current_symbol.declaration === DeclarationType.PersistentF ||
            current_symbol.declaration === DeclarationType.ActionF
        ) {
            ...
            if (name) {
                ...
                if (!(name.charCodeAt(0) >= 65 && name.charCodeAt(0) <= 90)) {
                    build_error_display(
                        ...
                        editor,
                        diags,
                        "Error: facts must start with an uppercase",
                    );
                }
                if (
                    current_symbol.declaration === DeclarationType.ActionF &&
                    current_symbol.context?.grammarType === "simple_rule"
                ) {
                ...
                if (
                    (current_symbol.declaration === DeclarationType.LinearF ||
                        current_symbol.declaration === DeclarationType.PersistentF) &&
                    current_symbol.node.parent?.grammarType === DeclarationType.Conclusion
                ) {
                    facts.push(current_symbol.name);
                    continue;
                }
                ...
                for (let j = 0; j < facts.length; j++) {
                    ...
                    if (name2) {
                        ...
                        if (distance < 3 && !facts.includes(current_symbol.name)) {
                            // threshold value
                            const diagnostic = build_warning_display(
                                current_symbol.node,
                                editor,
                                diags,
                                "Warning: did you mean " + name2 + " ? (" + distance + " characters away)",
                            );
...
function check_variable_is_defined_in_premise(
    symbol_table: TamarinSymbolTable,
    editor: vscode.TextEditor,
    diags: vscode.Diagnostic[],
): void {
    for (let i = 0; i < symbol_table.getSymbols().length; i++) {
        ...
        if (current_symbol.type === "$") {
            continue;
        } // Do not take into account public variables
        if (
            current_symbol.declaration === DeclarationType.CCLVariable ||
            current_symbol.declaration === DeclarationType.ActionFVariable
        ) {
            ...
            if (!is_break) {
                build_error_display(
                    ...
                    editor,
                    diags,
                    "Error: this variable is used in the second part of the rule but doesn't appear in premise",
                );
            }
        } else if (
            (current_symbol.declaration === DeclarationType.LinearF ||
                current_symbol.declaration === DeclarationType.PersistentF) &&
            current_symbol.node.parent?.grammarType === DeclarationType.Premise
        ) {
            let isbreak = false;
            for (const symbol of symbol_table.getSymbols()) {
                if (
                    ...
                    symbol.node.id !== current_symbol.node.id
                ) {
                    if (
            ...
            if (!isbreak) {
                build_error_display(
                    ...
                    editor,
                    diags,
                    "Error : fact occur in premise but never in any conclusion ",
                );
            }
        }
    }
            ...
            for (let j = 0; j < actionFacts.length; j++) {
                if (actionFacts[j].name === current_symbol.name) {
                    ...
                    if (!(actionFacts[j].arity === current_symbol.arity)) {
                        if (current_symbol.name) errors.push(current_symbol.name);
                    }
                } else {
                    continue;
                }
...
function check_function_macros_and_facts_arity(
    symbol_table: TamarinSymbolTable,
    editor: vscode.TextEditor,
    diags: vscode.Diagnostic[],
) {
    ...
    for (let i = 0; i < symbol_table.getSymbols().length; i++) {
        const current_symbol = symbol_table.getSymbol(i);
        if (
            symbol_table.getSymbol(i).declaration === DeclarationType.LinearF ||
            symbol_table.getSymbol(i).declaration === DeclarationType.PersistentF ||
            symbol_table.getSymbol(i).declaration === DeclarationType.Functions ||
            symbol_table.getSymbol(i).declaration === DeclarationType.NARY ||
            symbol_table.getSymbol(i).declaration === DeclarationType.Macro
        ) {
                            ...
                            if (current_symbol.arity === known_functions[k].arity) {
                                ...
                                if (current_symbol.declaration === DeclarationType.NARY) {
                                    build_error_display(
                                        ...
                                        editor,
                                        diags,
                                        "Error : incorrect arity for this function, " +
                                            known_functions[k].arity +
                                            " arguments required",
                                    );
                                } else if (
                                    current_symbol.declaration === DeclarationType.LinearF ||
            ...
            if (!isbreak) {
                ...
                for (const functionSymbol of known_functions) {
                    if (
                        ...
                        typeof functionSymbol.name === "string" /*condition to use them in the block*/ &&
                        ...
                        if (distance < 3) {
                            ...
                            const diagnostic = build_warning_display(
                                ...
                                editor,
                                diags,
                                "Warning: did you mean " + functionSymbol.name + " ? (" + distance + "characters away)",
                            );
                            diagnostic.code = "wrongFunctionName";
                            const range = symbol.name_range;
                            const fix = new vscode.CodeAction(
                                "Replace with " + functionSymbol.name,
...
function check_free_term_in_lemma(
    symbol_table: TamarinSymbolTable,
    editor: vscode.TextEditor,
    diags: vscode.Diagnostic[],
) {
    ...
    for (const symbol of symbol_table.getSymbols()) {
        if (
            symbol.declaration === DeclarationType.LemmaVariable ||
            symbol.declaration === DeclarationType.RestrictionVariable
        ) {
    ...
    for (let i = 0; i < lemma_vars.length; i++) {
        ...
        while (context?.grammarType !== "theory") {
            ...
            if (context?.children) {
                ...
                while (
                    search_context.child(0)?.grammarType === "conjunction" ||
                    search_context.child(0)?.grammarType === "disjunction" ||
                    gt_list.includes("imp")
                ) {
                ...
                if (
                    search_context.grammarType === DeclarationType.Lemma ||
                    search_context.grammarType === DeclarationType.Restriction ||
                    search_context.grammarType === "diff_lemma"
                ) {
            ...
            for (let j = 0; j < lemma_vars.length; j++) {
                if (
                    (lemma_vars[j].context?.id === context?.id ||
                        context_child_id.includes((lemma_vars[j].context as { id: number }).id)) &&
                    lemma_vars[j].node.parent?.grammarType === DeclarationType.QF &&
                    lemma_vars[j].name === lemma_vars[i].name
                ) {
                    isbreak = true;
                    break;
                }
        ...
        if (!globalisbreak) {
            build_warning_display(
                ...
                editor,
                diags,
                "Warning : free term in lemma or restriction formula",
            );
        }
    }
}
...
function check_macro_not_in_equation(
    symbol_table: TamarinSymbolTable,
    editor: vscode.TextEditor,
    diags: vscode.Diagnostic[],
) {
    for (const symbol of symbol_table.getSymbols()) {
        if (symbol.declaration === DeclarationType.NARY) {
            for (const macros of symbol_table.getSymbols()) {
                if (
                    macros.declaration === DeclarationType.Macro &&
                    macros.name === symbol.name &&
                    symbol.context.grammarType === DeclarationType.Equation
                ) {
                    build_error_display(
                        symbol.node,
                        editor,
                        diags,
                        "Error : a macro shoud not be used in an equation ",
                    );
                }
            }
        }
...
function get_builtins_name(builtins: TamarinSymbol[]): string[] {
    ...
    for (const builtin of builtins) {
        if (builtin.name) Sbuiltins.push(builtin.name);
    }
    return Sbuiltins;
}
...
function check_infix_operators(
    symbol_table: TamarinSymbolTable,
    editor: vscode.TextEditor,
    diags: vscode.Diagnostic[],
    ...
    function display_infix_error(builtin: string, symbol: string, child: Parser.SyntaxNode): void {
        ...
        if (
            !get_builtins_name(current_builtins).includes(builtin) &&
            !current_functions.includes(getName(child.child(0), editor))
        ) {
            const diagnostic = build_error_display(
                child,
                editor,
                diags,
                "Error : symbol " + symbol + " cannot be used without " + builtin + " builtin",
            );
            ...
            if (current_builtins.length > 0) {
                ...
                while (theory.grammarType !== "theory") {
                    if (theory.parent) theory = theory.parent;
                }
    ...
    for (const child of root.children) {
        if (child.grammarType === "^" || child.grammarType === "*") {
            ...
            if (getName(child.child(0), editor) === "inv") {
                ...
                getName(child.child(0), editor) === "revealSign" ||
                getName(child.child(0), editor) === "revealVerify" ||
                getName(child.child(0), editor) === "getMessage"
            ) {
