...
export function getName(node: Parser.SyntaxNode | null, editor: vscode.TextEditor): string {
    if (node && node.isNamed) {
        return editor.document.getText(
            new Range(editor.document.positionAt(node.startIndex), editor.document.positionAt(node.endIndex)),
        );
    } else {
        return "None";
    }
...
export async function detect_errors(editeur: vscode.TextEditor): Promise<Parser.SyntaxNode | void> {
    ...
    function build_error_display(node: Parser.SyntaxNode, editeur: vscode.TextEditor, message: string) {
        ...
        const existingDiag = diags.find((d) => d.range.contains(positionStart)); // Evite les superpositions de diagnostics
        if (!existingDiag) {
            diags.push(
                new vscode.Diagnostic(
                    new vscode.Range(positionStart, positionStart.translate(0, 0)),
                    message,
    ...
    function typesOfError(node: Parser.SyntaxNode, editeur: vscode.TextEditor) {
        if (
            node.grammarType === "theory" ||
            node.nextSibling?.nextSibling?.grammarType === "begin" ||
            node.nextSibling?.grammarType === "begin"
        ) {
            build_error_display(node, editeur, "MISSING 'theory' or 'begin'");
        } else if (
            node.firstChild?.grammarType === "builtins" ||
            node.firstChild?.grammarType === "functions" ||
            node.firstChild?.grammarType === "macros"
        ) {
            build_error_display(node, editeur, "MISSING ':' ");
        } else if (node.nextSibling?.grammarType === "built_in") {
            build_error_display(node, editeur, "MISSING ',' ");
        } else if (
            node.firstChild?.grammarType === "rule" &&
            (node.firstChild.nextSibling?.grammarType != "ident" ||
                node.firstChild.nextSibling?.nextSibling?.grammarType != ":")
        ) {
            ...
            node.firstChild?.grammarType === "premise" ||
            (node.firstChild?.grammarType === "rule" &&
                (node.firstChild.nextSibling?.grammarType === "ident" ||
                    node.firstChild.nextSibling?.nextSibling?.grammarType === ":"))
        ) {
            build_error_display(
                node,
                editeur,
                "ERROR in rule structure the syntax for a rule is either \n []-->[] \n or \n []--[]->[]",
            );
        } else if (node.firstChild?.grammarType === "pre_defined") {
            build_error_display(node, editeur, "MISSING generalized quantifier");
        } else if (
            node.firstChild?.grammarType === "nested_formula" ||
            node.firstChild?.grammarType === "action_constraint" ||
            node.firstChild?.grammarType === "conjunction"
        ) {
    ...
    function findMatches(node: Parser.SyntaxNode, editeur: vscode.TextEditor) {
        if (node.isMissing) {
            ...
            if (node.parent?.child(myId - 1)?.type === "single_comment") {
                ...
                const positionStart = editeur.document.positionAt(start ?? 0);
                const existingDiag = diags.find((d) => d.range.contains(positionStart));
                if (!existingDiag) {
                    diags.push(
                        new vscode.Diagnostic(
                            new vscode.Range(positionStart, positionStart.translate(0, 1)),
                            node.toString().slice(1, -1),
                            vscode.DiagnosticSeverity.Error,
                        ),
                    );
                }
