...
const diagCollection = vscode.languages.createDiagnosticCollection("Tamarin");
export const createSymbolTable = async (
    root: Parser.SyntaxNode,
    editor: vscode.TextEditor,
): Promise<CreateSymbolTableResult> => {
...
function find_variables(node: Parser.SyntaxNode): Parser.SyntaxNode[] {
    ...
    for (const child of node.children) {
        ...
        if (
            child.grammarType === "pub_var" ||
            child.grammarType === "fresh_var" ||
            child.grammarType === DeclarationType.MVONF ||
            child.grammarType === "nat_var" ||
            child.grammarType === "temporal_var"
        ) {
            vars.push(child);
...
function find_linear_fact(node: Parser.SyntaxNode): Parser.SyntaxNode[] {
    ...
    for (const child of node.children) {
        ...
        if (
            child.grammarType === DeclarationType.LinearF ||
            child.grammarType === DeclarationType.NARY ||
            child.grammarType === DeclarationType.PersistentF
        ) {
            vars.push(child);
...
export function get_range(node: Parser.SyntaxNode | null, editor: vscode.TextEditor): vscode.Range {
    if (node) {
        const startPos = editor.document.positionAt(
            editor.document.offsetAt(new vscode.Position(node.startPosition.row, node.startPosition.column)),
        );
        const endPos = editor.document.positionAt(
            editor.document.offsetAt(new vscode.Position(node.endPosition.row, node.endPosition.column)),
        );
...
function convert(grammar_type: string): DeclarationType {
    if (grammar_type === "nary_app") {
        return DeclarationType.NARY;
    } else if (grammar_type === "linear_fact") {
        return DeclarationType.LinearF;
    } else {
        return DeclarationType.DEFAULT;
    }
}
...
class SymbolTableVisitor {
    ...
    public async visit(
        ...
        if (this.visitcounter === 0) {
            ...
            for (let k = 0; k < AssociatedFunctions[AssociatedFunctions.length - 1].length; k += 2) {
                this.registerfucntion(
                    ...
                    AssociatedFunctions[AssociatedFunctions.length - 1][k],
                    parseInt(AssociatedFunctions[AssociatedFunctions.length - 1][k + 1]),
                    root,
                    get_range(root, editor),
                );
            }
        }
        ...
        for (let i = 0; i < root.children.length; i++) {
            ...
            if (
                child?.grammarType === DeclarationType.Lemma &&
                (root.grammarType === "lemma" || root.grammarType === "diff_lemma") &&
                root.parent !== null
            ) {
                this.registerident(
                    root,
                    DeclarationType.Lemma,
                    getName(child?.nextSibling, editor),
                    root.parent,
                    get_range(child?.nextSibling, editor),
                );
                ...
                child?.grammarType === DeclarationType.Restriction &&
                root.grammarType === "restriction" &&
                root.parent !== null
            ) {
                this.registerident(
                    root,
                ...
                child?.grammarType === DeclarationType.Rule &&
                root.grammarType === "simple_rule" &&
                root.parent !== null
            ) {
                this.registerident(
                    root,
                ...
                for (const grandchild of child.children) {
                    if (
                        grandchild.grammarType === DeclarationType.FUNCP ||
                        grandchild.grammarType === DeclarationType.FUNCPR ||
                        grandchild.grammarType === DeclarationType.FUNCD ||
                        grandchild.grammarType === DeclarationType.FUNCUST
                    ) {
                        this.registerfucntion(
                            grandchild,
                            DeclarationType.Functions,
                            getName(grandchild.child(0), editor),
                            parseInt(getName(grandchild.child(2), editor)),
                            root,
                            get_range(grandchild.child(0), editor),
                        );
                    }
                }
            } else if (
                child?.grammarType === DeclarationType.Macros ||
                child?.grammarType === DeclarationType.Equations
            ) {
                for (const grandchild of child.children) {
                    if (grandchild.grammarType === DeclarationType.Macro) {
                        this.registerfucntion(
                            ...
                            getName(grandchild.child(0), editor),
                            get_macro_arity(grandchild.children),
                            root,
                            get_range(grandchild.child(0), editor),
                        );
                        ...
                        for (const ggchild of grandchild.children) {
                            ...
                            if (eqcount === 0) {
                                this.register_vars_left_macro_part(
                                    ggchild,
                                    DeclarationType.LMacroVariable,
                                    editor,
                                    grandchild,
                                );
                            } else {
                ...
                for (const grandchild of child.children) {
                    if (grandchild.grammarType === DeclarationType.Builtin && grandchild.child(0) !== null) {
                        ...
                        this.registerident(
                            grandchild,
                            DeclarationType.Builtin,
                            builtinType,
                            root,
                            get_range(grandchild, editor),
                        );
                        ...
                        if (built_in_index >= 0) {
                            for (let k = 0; k < AssociatedFunctions[built_in_index].length; k += 2) {
                                ...
                                this.registerfucntion(
                                    grandchild,
                                    DeclarationType.Functions,
                                    AssociatedFunctions[built_in_index][k],
                                    parseInt(AssociatedFunctions[built_in_index][k + 1]),
                                    root,
                                    get_range(grandchild, editor),
                                );
                            }
                        ...
                        if (
                            builtinType === "asymmetric-encryption" ||
                            builtinType === "signing" ||
                            builtinType === "revealing-signing"
                        ) {
                            pkcount++;
                        }
                    }
            ...
            if (vars[k].grammarType === DeclarationType.MVONF) {
                ...
                for (const symbol of this.symbolTable.getSymbols()) {
                    if (symbol.declaration === DeclarationType.Functions) {
                        if (symbol.name === getName(vars[k], editor)) {
                            ...
                            this.registerfucntion(
                                vars[k],
                                DeclarationType.NARY,
                                symbol.name,
                                0,
                                root,
                                get_range(vars[k], editor),
                            );
                        }
                    } else {
                        continue;
                ...
                if (!isregistered) {
                    this.registerident(
                        vars[k],
                        type,
                        getName(vars[k].child(0), editor),
                        root,
                        get_range(vars[k].child(0), editor),
                    );
                }
            } else {
                this.registerident(
                    vars[k],
                    type,
                    getName(vars[k].child(1), editor),
                    root,
                    get_range(vars[k].child(1), editor),
                    vars[k].child(0)?.grammarType,
                );
            }
        }
    }

    private register_vars_left_macro_part(
        node: Parser.SyntaxNode,
        type: DeclarationType,
        editor: vscode.TextEditor,
        root: Parser.SyntaxNode,
        ...
        if (node.grammarType === DeclarationType.MVONF) {
            ...
            node.grammarType === "pub_var" ||
            node.grammarType === "fresh_var" ||
            node.grammarType === "nat_var" ||
            node.grammarType === "temporal_var"
        ) {
            this.registerident(
                node,
                type,
                getName(node.child(1), editor),
                root,
                get_range(node.child(1), editor),
                node.child(0)?.grammarType,
            );
        }
    }
    ...
    private register_vars_lemma(node: Parser.SyntaxNode, type: DeclarationType, editor: vscode.TextEditor) {
        ...
        for (let k = 0; k < vars.length; k++) {
            ...
            while (
                context.grammarType !== DeclarationType.NF &&
            ...
            if (vars[k].parent !== null) {
                if (
                    vars[k].grammarType === DeclarationType.MVONF ||
                    (vars[k].grammarType === DeclarationType.TMPV && vars[k].children.length === 1)
                ) {
                    let isregistered = false;
                    for (const symbol of this.symbolTable.getSymbols()) {
                        if (symbol.declaration === DeclarationType.Functions) {
    ...
    private register_facts_searched(
        node: Parser.SyntaxNode,
        editor: vscode.TextEditor,
        root: Parser.SyntaxNode,
        type?: DeclarationType,
    ...
    private registerident(
        ident: Parser.SyntaxNode | null | undefined,
